...
def to_bin(num, l):
    ...
    if int(num) < 0:
        sign = "1"
        signed_number = 2 ** int(l) + int(num)
        bin_num = bin(signed_number)
    else:
        bin_num = bin(int(num))

    bin_num = bin_num[2:]
    while len(bin_num) < l:
        bin_num = sign + bin_num
...
def assembly_to_hex(asm_dir, hex_dir):
    ...
    for i in range(len(file_list)):
        ...
        for line in asm_lines:
            ...
            #2 reg load and store instructions (rt in front of rs)
            elif i_line[0].upper() in ['LB', 'LBU', 'LH', 'LHU', 'LW', 'LWL', 'LWR', 'SB', 'SH', 'SW']:
                rt = to_bin(i_line[1],5)
                i_data = to_bin(i_line[2], 16) if len(i_line) == 4 else to_bin(0, 16)
                rs = to_bin(i_line[3],5) if len(i_line) == 4 else to_bin(i_line[2],5)
            ...
            #1 reg (rs)
                ...
                rs = to_bin(i_line[1],5)
                i_data = to_bin(i_line[1],16) if len(i_line) == 3 else to_bin(0, 16) #index needed to be 1 rather than 2 as there is only one arguement
            #1 reg (rt)
            ...
            #branch z instructions
                ...
                rs = to_bin(i_line[1],5)
                rt = branch_s2_codes[i_line[0].upper()]
                i_data = to_bin(i_line[2],16) if len(i_line) == 3 else to_bin(0, 16)
            ...
            # might assume that it is 31!!
            elif i_line[0].upper() == 'JALR':
                rs = to_bin(i_line[2],5) if len(i_line) == 3 else to_bin(i_line[1],5)
                rd = to_bin(i_line[1],5) if len(i_line) == 3 else to_bin(0, 5)
            ...
            with open(os.path.join(hex_dir, filename), 'a') as hexed_file:
                hexed_file.write(hex_machine_code[:2] + "\n")
                hexed_file.write(hex_machine_code[2:4] + "\n")
                hexed_file.write(hex_machine_code[4:6] + "\n")
                hexed_file.write(hex_machine_code[6:8]+ "\n")
            hexed_file.close()
        #Filler zeroes to fill remaining locations in range 20000-40000    
        #print(count)
        with open(os.path.join(hex_dir, filename), 'a') as hexed_file:
            for y in range(20000 - (count*4) -1):
                hexed_file.write("00" + "\n")
            hexed_file.write("00")
        hexed_file.close()
